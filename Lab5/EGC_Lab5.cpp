...
void initPolygon() {
    ...
    p.addVertex(vec3(400.0f, 400.0f, 1.0f));
    p.addVertex(vec3(100.0f, 400.0f, 1.0f));
...
void processEvents() {
    ...
    if (SDL_PollEvent(&currentEvent) != 0) {
        ...
        if (currentEvent.type == SDL_KEYDOWN) {
            ...
            case SDLK_UP:
                ty -= 1.0f;
                p.transformationMatrix = translate(tx, ty);
                break;
          ...
          case SDLK_a:
                angle -= 1.0f;
                p.transformationMatrix = translate(p.getCx(), p.getCy()) * rotate(angle) * translate(-p.getCx(), -p.getCy());
                break;
...
void drawFrame() {
   ...
   // p.transformationMatrix = translate(p.getCx(), p.getCy()) * rotate(tx) * translate(-p.getCx(), -p.getCy());
